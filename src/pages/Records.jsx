  ...
  return (
    <div className="min-h-full bg-background" onTouchStart={handlePullStart}>
      ...
      <Drawer open={showBadges} onOpenChange={setShowBadges}>
        <DrawerContent className="max-h-[80vh] overflow-y-auto">
          ...
          {filteredBadges.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground"><p className="text-3xl mb-2">🦊</p><p className="text-sm">아직 획득한 칭호가 없어요.</p></div>
          ) : (
                    ...
                    <div className="space-y-2">
                      {catBadges.map(b => (
                        <div key={b.id} className="p-3 rounded-xl bg-amber-50/80 border border-amber-200/60">
                          ...
                          {b.earned_date && <p className="text-[10px] text-amber-600 mt-1">{b.earned_date}</p>}
                        </div>
                      ))}
                    </div>
                  </div>
      ...
      <Drawer open={showSessions} onOpenChange={setShowSessions}>
        <DrawerContent className="max-h-[80vh] overflow-y-auto">
          ...
          {filteredLogs.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground"><p className="text-3xl mb-2">🦊</p><p className="text-sm">아직 수련 기록이 없어요.</p></div>
          ) : (
                ...
                return (
                  <button key={log.id} onClick={() => setSelectedPhoto(log)}
                    ...
                    <div className="flex items-center gap-2">
                      {log.duration_minutes > 0 && <span className="text-xs text-muted-foreground">⏱️ {log.duration_minutes}분</span>}
                      {log.gps_enabled && <span className="text-xs text-blue-600">🗺️ {log.distance_km?.toFixed(2)}km</span>}
                    </div>
                  </button>
                );
              })}
      ...
      <Drawer open={showCompletedGoals} onOpenChange={setShowCompletedGoals}>
        <DrawerContent className="max-h-[80vh] overflow-y-auto">
          ...
          {completedGoalsList.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground"><p className="text-3xl mb-2">🦊</p><p className="text-sm">아직 완료한 목표가 없어요.</p></div>
          ) : (
            <div className="space-y-5 mt-1 px-4 pb-6">
              {['exercise', 'study', 'mental', 'daily'].map(cat => {
                ...
                return (
                  <div key={cat}>
                    <p className="text-xs font-bold text-amber-800 mb-2">{info.emoji} {info.label} ({catGoals.length}개)</p>
                    <div className="space-y-2">
                      {catGoals.map(g => {
                        ...
                        return (
                          <div key={g.id} className="p-4 rounded-xl bg-gradient-to-r from-amber-50 to-amber-100/60 border border-amber-300/60">
                            <p className="text-sm font-bold text-amber-900 mb-1">{g.title}</p>
                            <p className="text-xs text-muted-foreground mb-1">{g.duration_days}일 도전 · {g.end_date || g.updated_date?.split('T')[0]}</p>
                            {relBadge && <p className="text-xs text-amber-700 font-semibold">🏅 {relBadge.title}</p>}
                            {g.result_note && <p className="text-xs text-amber-800 italic mt-1">"{g.result_note}"</p>}
        ...
        <DrawerContent className="max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-4 mt-2 px-4 pb-6">
            {(() => {
              ...
              if (!distanceLogs.length) return <div className="text-center py-8 text-muted-foreground"><p className="text-3xl mb-2">🗺️</p><p className="text-sm">저장된 GPS 기록이 없어요.</p></div>;
              const byCategory = {};
              ...
              return Object.entries(byCategory).map(([cat, catLogs]) => {
                ...
                return (
                  <div key={cat}>
                    <p className="text-xs font-bold text-amber-800 mb-2">{CAT_EMOJIS[cat]} {CAT_LABELS[cat]} - {catDistance}km</p>
                    <div className="space-y-2">
                      {catLogs.map(log => {
                        const ag = actionGoals.find(a => a.id === log.action_goal_id);
                        return (
                          <button key={log.id} onClick={() => setSelectedPhoto(log)}
                            ...
                            <div className="flex items-center justify-between gap-2 mb-1">
                              <p className="text-sm font-semibold text-amber-900">{ag?.title || '기록'}</p>
                              <span className="text-[10px] px-2 py-0.5 rounded-full bg-blue-200 text-blue-800 font-semibold">{log.distance_km?.toFixed(2)}km</span>
                            </div>
      ...
      <div className="p-6 pb-3">
        <h1 className="text-xl font-bold text-amber-900">📜 기록의 여정</h1>
      </div>
                ...
                {catBreakdown.map(([cat, min]) => {
                  ...
                  return (
                    <div key={cat} className="flex items-center gap-3">
                      ...
                      <span className="text-xs text-muted-foreground w-16 text-right">{Math.round(min / 60)}시간 {percent}%</span>
                    </div>
                  );
                })}
              </div>
        ...
        <>
          ...
          <Dialog open={!!selectedPhoto} onOpenChange={() => setSelectedPhoto(null)}>
            <DialogContent className="max-w-sm rounded-2xl p-4">
              <DialogHeader>
                <DialogTitle className="text-center text-sm">{CAT_EMOJIS[selectedPhoto?.category]} {selectedPhoto?.date}</DialogTitle>
              </DialogHeader>
              {selectedPhoto && (
                <div className="space-y-3">
