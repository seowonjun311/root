...
export function getObjectScreenPosition(item, kind) {
  ...
  if (kind === 'decoration' && item?.type === 'atlantis_temple') return { x: x + 4, y: y + TILE_H * 4 - 35 };
  if (kind === 'decoration' && item?.type === 'atlantis_palace') return { x: x - 5, y: y + TILE_H * 5 - 129 };
  ...
  if (kind === 'decoration' && item?.type === 'market_stall') {
  ...
  if (kind === 'decoration' && item?.type === 'basic_lamp') return { x, y: y + TILE_H * 2 - 75 };
