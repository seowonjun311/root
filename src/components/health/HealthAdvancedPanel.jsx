    ...
    <div className="mx-4 mb-4">
      ...
      {panelOpen && (
        <div className="mt-2 space-y-2">
          {FEATURES.map(({ key, label, emoji, icon: Icon, desc, color, bg }) => {
            const isOn = !!settings[key];
            const isExpanded = !!expanded[key];

            return (
              <div key={key} className={`rounded-2xl border border-border overflow-hidden ${isOn ? '' : 'opacity-70'}`}>
                {/* 기능 헤더 */}
                <div className={`flex items-center gap-3 px-4 py-3 ${isOn ? bg : 'bg-secondary/20'}`}>
                  <span className="text-lg">{emoji}</span>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-foreground">{label}</p>
                    {!isOn && <p className="text-[11px] text-muted-foreground">{desc}</p>}
                  </div>
                  <div className="flex items-center gap-2">
                    {/* ON/OFF 토글 */}
                    <button
                      onClick={() => toggleFeature(key)}
                      className={`relative w-11 h-6 rounded-full transition-colors duration-200 ${isOn ? 'bg-primary' : 'bg-border'}`}
                    >
                      <span
