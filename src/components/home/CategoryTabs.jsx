  ...
  return (
    <div className="flex gap-2 px-4 py-3">
      {categories.map(({ id, label, emoji }) => {
        ...
        return (
          <div key={id} className="flex-1 flex flex-col gap-1">
            {/* 레벨 표시 */}
            <div className="text-center px-1 py-0.5 rounded bg-black/40 backdrop-blur-sm border border-amber-600/40">
              <span className="text-[10px] font-bold text-amber-100">Lv.{level}</span>
            </div>
            {/* 버튼 */}
            <button
              onClick={() => onChange(id)}
