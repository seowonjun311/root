...
export default function ActionGoalCard({ actionGoal, weeklyLogs = [], onComplete }) {
  ...
  const { data: user, isLoading: isUserLoading } = useQuery({
    queryKey: ['me'],
    queryFn: () => Promise.resolve(null), // guest-safe placeholder
    enabled: false,
  });
    ...
    if ('geolocation' in navigator) {
      watchIdRef.current = navigator.geolocation.watchPosition(
        ...
        (error) => {
          ...
          toast.error('위치 추적을 시작할 수 없어요. 위치 권한을 확인해주세요.');
        },
        {
  ...
  const updateMutation = useMutation({
    ...
    onMutate: async (updateData) => {
      await queryClient.cancelQueries({ queryKey: actionGoalsKey });
      const previous = queryClient.getQueryData(actionGoalsKey);

      queryClient.setQueryData(actionGoalsKey, (old = []) =>
    ...
    onError: (_err, _vars, context) => {
      if (context?.previous) {
        queryClient.setQueryData(actionGoalsKey, context.previous);
      }
  ...
  const typeLabel =
    actionGoal.action_type === 'timer'
      ? actionGoal.duration_minutes
        ? `${actionGoal.duration_minutes}분`
        : ''
      : '';
  ...
  return (
    <>
      <div ref={cardRef} className="mx-4 relative">
          ...
          <div className="px-4 pt-3 pb-2">
            <div className="flex items-center justify-between mb-2.5">
              ...
              >
                ...
                <span className="font-bold text-sm truncate" style={{ color: '#3a1f04' }}>
                  {actionGoal.title}
                </span>
                {typeLabel && (
                  <span className="text-xs ml-1" style={{ color: 'var(--wood-primary-dark)' }}>
                    {typeLabel}
                  </span>
                )}
                <span
                  className="text-xs font-bold ml-1 shrink-0"
                  style={{ color: 'var(--wood-primary-darker)' }}
                >
              ...
              <div className="flex items-center gap-1.5 ml-2">
                {actionGoal.action_type === 'timer' ? (
                  doneToday && !isRunning ? (
                    <span
                      className="h-8 px-3 text-xs font-bold rounded-md flex items-center gap-1"
                      style={{
                      ...
                      <button
                        ...
                        style={
                          isRunning
                            ...
                            : {
                                background:
                                  'linear-gradient(180deg, var(--wood-primary) 0%, var(--wood-primary-dark) 50%, var(--wood-primary-darker) 100%)',
            ...
            >
              <div
                ...
                style={{
                  ...
                  background:
                    'linear-gradient(90deg, var(--wood-primary-darker) 0%, var(--wood-primary) 50%, #e8c060 100%)',
        ...
        <AnimatePresence>
          {showCalendar && (
            <MonthCalendar weeklyLogs={weeklyLogs} onClose={() => setShowCalendar(false)} />
          )}
        </AnimatePresence>
      </div>
      ...
      <Drawer open={showEdit} onOpenChange={setShowEdit}>
        <DrawerContent>
          <DrawerHeader className="text-center">
            <DrawerTitle>✏️ 행동 목표 수정</DrawerTitle>
          </DrawerHeader>
          <div className="px-4 space-y-4 pb-6">
            <div>
              <label className="text-xs font-semibold text-amber-800 mb-1.5 block">
                행동 목표 이름
              </label>
      ...
      <Drawer open={showGpsDialog} onOpenChange={setShowGpsDialog}>
        <DrawerContent>
          <DrawerHeader className="text-center">
            <DrawerTitle>🗺️ GPS 추적을 하시겠습니까?</DrawerTitle>
          </DrawerHeader>
          <p className="px-4 text-sm text-muted-foreground text-center">
